...
public class MazeManager : MonoBehaviour {
    ...
    public void SaveMap () {
        ...
        bool flag = false;
        PrefabUtility.SaveAsPrefabAsset (mapGroup, "Assets/Prefabs/Maps/" + mapName + "_" + mapCount.ToString () + ".prefab", out flag);
        if (flag) {
            mapCount++;
    ...
    public void ResetCamera () {
        ...
        if (_camera.orthographic) {
            ...
            mainCamera.transform.position = new Vector3 (
                mapGroup.transform.position.x,
                _camera.orthographicSize,
                mapGroup.transform.position.z
            );
        } else { // 透视相机
